# Hi, I'm Julius 👋

**AI Safety Researcher & Full-Stack Engineer** building responsible AI systems that work in the real world, with a focus on African contexts.

## What I Do

- 🛡️ **AI Safety & Governance** — Contributing to global AI safety research coordination through [SPAR](https://sparai.org) and building ethical AI frameworks
- 🏗️ **Product & Engineering** — 8+ years shipping production systems in healthcare, governance, legal tech, and recruitment
- 🎓 **Learning** — Currently pursuing MSc Data Science at University of Ghana, focusing on AI alignment and responsible AI systems
- 🌍 **Impact** — Built systems that reduced legal licensing from 2 months to 5 minutes, and governance tools used by multinational organizations globally

## Current Work

- Building **AI Safety Connect** — a platform mapping global AI safety researchers and organizations
- Developing AI-powered health insights at **CuraAlert** with rigorous safety considerations
- Leading AI governance initiatives at **TheBoardroom Africa**
- Maintaining [CIRIS](https://ciris.ai) — an AI ethical framework (Compliance, Inclusion, Responsibility, Integrity, Safety)

## Tech Stack

**Languages:** Python, JavaScript, TypeScript, PHP  
**Frontend:** React, Next.js, Tailwind CSS  
**Backend:** Node.js, Laravel  
**AI/ML:** TensorFlow, PyTorch, Deep Learning, Neural Networks  
**Cloud & Tools:** AWS, GCP, Azure, Vercel, Git, MySQL  

## Philosophy

I believe AI systems should be:
- **Safe by design** — alignment and governance from day one
- **Transparently deployed** — users should understand how systems affect them
- **Contextually appropriate** — especially critical in emerging markets where governance frameworks are still developing
- **Actually useful** — real-world impact over perfect prototypes

## Notable Projects

- 🏛️ GovernanceBaseline — Data-driven board performance evaluation platform
- ⚖️ Legal Licensing System — Reduced processing time from 60 days to 5 minutes
- 🏥 Health Insurance Management — Streamlined policy administration and claims processing
- 📚 Examination Management System — Automated student progression, resits, and academic decisions
- 🔧 DevLess — Backend-as-a-service platform with 140,000+ downloads

## Recent Contributions

- **Future of Life Institute** — Built searchable documentation for "Keep the Future Human" essay
- **AI Governance Policy** — Authored frameworks for responsible AI in recruitment systems
- **AI Safety Ecosystem** — Architecting platforms to improve global safety research collaboration

## Community

- [AI Safety Collab](https://www.aisafetycollab.com/)
- [BlueDot Safety Network](https://bluedot.org/)
- AI Alignment communities
- Future of Life Institute contributor

## Let's Connect

- 💼 [LinkedIn](https://www.linkedin.com/in/julius-odai)
- 📧 jlsodai@gmail.com
- 🌐 [Portfolio](https://bit.ly/jaoPortfolio)
- 🏢 [TheBoardroom Africa](https://theboardroomafrica.com)

---

*Building AI systems that actually help people, with safety and accountability baked in from the start.*
